...
const previewSheet = {
	...
	PreviewSheetCtrl.$inject = ["$scope", "$state", "$timeout", "$window", "$mdDialog"];

	function PreviewSheetCtrl($scope, $state, $timeout, $window, $mdDialog) {
		var $ctrl = this;
		$ctrl.close = function () {
			$mdDialog.hide();
		};
		...
		$scope.$watch("$ctrl.itemId", function (newValue, oldValue) {
			...
			$mdDialog.show({
				...
				controller: [
					...
					"$state",
					// "openEvent",
					function signatureConfirmationCtrlInner(
						$mdDialog,
						itemId,
						$state
						// openEvent
